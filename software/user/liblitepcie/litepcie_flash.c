 ...
 * Matches openFPGALoader wait_ready() behavior. */
static int bpi_wait_ready(int fd, uint32_t timeout_ms)
{
    ...
    while (elapsed < timeout_ms) {
        ...
        if (sr & BPI_SR_READY) {
            ...
            /* Return to read array mode (matching openFPGALoader). */
 ...
 * Matching openFPGALoader: usleep(100) between commands, no wait_ready. */
                     ...
                     void *opaque)
{
     ...
     * Per-chunk sequence (matching openFPGALoader):
     *   1. Clear status (0x50)
